<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <customers-view-sidebar :isSidebarActive="addNewDataSidebar" @closeSidebar="toggleDataSidebar" :data="sidebarData" />
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="customer-name font-medium truncate">{{ `${tr.first_name} ${tr.last_name}` }}</p>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    deleteDialog(id) {
      if (!this.loading && this.loadingIndex !== id) {
        this.$vs.dialog({
          ...
          color: 'danger',
          title: `Confirmação`,
          text: 'Essa ação não poderá ser desfeita, tem certeza que deseja deletar o cliente?',
          acceptText: 'Deletar',
          cancelText: 'Cancelar',
    ...
    deleteData(id) {
      ...
      this.$store.dispatch('customers/removeItem', id)
      .then(({ data }) => {
          this.$vs.notify({
            color: 'success',
            text: data.message,
